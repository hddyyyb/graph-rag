...
Although the system already supports **structure-aware graph retrieval**, there are still several limitations:

- current graph retrieval is based on:
  - **weighted 1-hop term expansion** over co-occurrence graph
  - scoring driven by edge weights and hit counts

- not yet supported:
  - deeper multi-hop traversal (>1 expansion step)
  - path-aware reasoning (multi-step graph scoring)
  - entity-level graph modeling (currently term-level only)

- chunking strategy is still basic:
  - no recursive / semantic chunking
  - limited metadata (structure, section, hierarchy)


#### Retrieval Evaluation

The system currently lacks **quantitative evaluation capability**:

- no offline benchmark dataset
- no evaluation metrics implemented:
  - Recall@K
  - MRR
- no comparison across retrieval strategies:
  - vector vs graph vs hybrid


#### Infrastructure Limitations

The current system is not yet production-grade in infrastructure:

- SQLite vector store:
  - not scalable
  - no ANN indexing
- no production-grade vector database:
  - (Milvus / Qdrant not integrated yet)
- no full system deployment:
  - missing Docker Compose (API + vector DB + graph DB)
